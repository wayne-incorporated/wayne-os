...
constexpr char kLogTag[] = "ArcKeyMint ";

// Removes the file path from a given log message.
//
// Messages coming from the keymint logger may include a file path, which is
...
KeyMintLogger::KeyMintLogger() {
  set_instance(this);
}

int KeyMintLogger::log_msg(LogLevel level,
                           const char* fmt,
                           va_list args) const {

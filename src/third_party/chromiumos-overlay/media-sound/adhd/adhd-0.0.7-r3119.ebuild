...
EAPI=7
CROS_WORKON_COMMIT=("c0d9c8bc2fdeca079d482f3da37ceec3cd0e4605" "767ccd235023de9d859321727e75cc4451fb70ba")
CROS_WORKON_TREE=("12b4e85e50087e1f4cab766a3a5bdd0415755244" "9968650f80898498a22879c71e2d6a494227107c")
...
inherit cros-unibuild libchrome systemd user
...
bazel_external_uris="
	...
	https://github.com/abseil/abseil-cpp/archive/refs/tags/20230125.3.zip -> abseil-abseil-cpp-20230125.3.zip
...
LICENSE="BSD-Google"
KEYWORDS="*"
IUSE="asan +cras-apm cras-debug cras-ml cros-debug dlc featured fuzzer neon selinux systemd"
...
src_configure() {
	...
	common_bazel_args=(
		...
		"$(use_label cros-debug //cras/platform:cros_debug)"
	)
...
src_install() {
	...
	if ! use fuzzer ; then
		...
		fuzzer_install "${S}/adhd/OWNERS.fuzz" "${T}/dist/fuzzer"/cras_rclient_message_fuzzer \
			--comp "${fuzzer_component_id}"
		fuzzer_install "${S}/adhd/OWNERS.fuzz" "${T}/dist/fuzzer"/cras_hfp_slc_fuzzer \
			--dict "${S}/adhd/cras/src/fuzz/cras_hfp_slc.dict" \
			--comp "${fuzzer_component_id}"
		local fuzzer_component_id="769744"
		fuzzer_install "${S}/adhd/OWNERS.fuzz" "${T}/dist/fuzzer"/cras_fl_media_fuzzer \
			--comp "${fuzzer_component_id}"
	fi
}

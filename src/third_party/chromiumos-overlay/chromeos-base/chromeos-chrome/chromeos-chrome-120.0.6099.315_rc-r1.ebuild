...
add_api_keys() {
    ...
    # Added
    elog "This step is add_api_keys(). Debugging by seongbin@wayne-inc.com"
    elog "google_api_key=${api_key}"
    elog "google_default_client_id=${client_id}"
    elog "google_default_client_secret=${client_secret}"
    # by seongbin@wayne-inc.com

}

src_prepare() {
    # Added by seongbin@wayne-inc.com
    elog "elog: This step is src_prepare(). Debugging by seongbin@wayne-inc.com"
	...
	# Is this a bug? emerge-reven chromeos-chrome doesn't embed the googleapikeys with the original condition.
	# if [[ ! -d google_apis/internal ]]; then
	if [[ ! -d google_apis/internal-1 ]]; then
        # Added by seongbin@wayne-inc.com
        elog "elog: This step is src_prepare - if(google_apis/internal). Debugging by seongbin@wayne-inc.com"
		...
		local GAPI_CONFIG_FILE=${PRIVATE_OVERLAYS_DIR}/chromeos-overlay/googleapikeys-liwonace-release
		if [[ ! -f "${GAPI_CONFIG_FILE}" ]]; then
			# Then developer credentials.
			# Modified by seongbin@wayne-inc.com
			# GAPI_CONFIG_FILE=/home/${WHOAMI}/.googleapikeys
			GAPI_CONFIG_FILE=/home/${WHOAMI}/.googleapikeys-sysnesia-release
		fi
		if [[ -f "${GAPI_CONFIG_FILE}" ]]; then
